...
function* syncBlueprints(
  ...
  blueprints.map((blueprint: BluePrint) => {
    if (
      localBlueprints.filter(local => local.uid === blueprint.uid).length === 0
    ) {
...
function* doSync(): any {
  ...
  try {
    ...
    const result = yield call(api.sync, user.jwt);
    console.log({ result });
    ...
    if (result.data.collection) {
      yield syncCollection(result.data, user);
    }

    if (result.data.squadrons) {
      yield syncSquadrons(result.data, user);
    }

    if (result.data.blueprints) {
      yield syncBlueprints(result.data, user);
    }

    if (result.data.tournaments) {
      yield syncTournaments(result.data, user);
    }
